...
> `<keep-alive>`是Vue中内置的一个抽象组件，它自身不会渲染一个DOM元素，也不会出现在父组件链中。当它包裹动态组件时，会缓存不活动的组件
实例，而不是销毁它们。

`<keep-alive>`组件可以保留组件的状态或避免多次重新渲染，以提高页面性能

`<keep-alive>`接受三个属性：
- `include` 字符串或正则表达式，只有匹配上的组件会被缓存
- `exclude` 字符串或正则表达式，任何被匹配上的组件都不会被缓存
- `max` 数字，最多可以缓存多少组件实例

`<keep-alive>`不再执行`created`、`mounted`等钩子函数，其提供了另外两个钩子：`activated`和`deactivated`。
它们的执行时机时`<keep-alive>`包裹的组件激活时调用和停用是调用

### 主要原理

1. Vue会创建一个`cache`用于缓存组件，通过key-value的行式存储。同时创建一个`keys`数组，用于存放所有组件的key值
2. 获取节点名称，并与`include`和`exclude`进行匹配，如果组件不需要缓存，则直接返回这个组件的`vnode`
3. 如果组件需要缓存，则先判断组件是否已经在`cache`中被缓存，如果有则直接从缓存中拿该组件的实例，同时调整该组件的key的顺序，
将其从原来的地方删掉并重新放在`keys`的最后一位。
4. 如果缓存中没有，则将该组件存入`cache`，同时也将key值push到`keys`末尾。接着再判断此时缓存的组件数量是否超过了设置的最大值，
如果超过了，则删除第一个缓存的组件。

此处应用了缓存淘汰策略*LRU*

> LRU(Least recently used)最近最少使用算法，其核心思想是“如果数据最近被访问过，那么将来被访问的几率也更高”

如下图所示：
![avater](/vue-1.png)

1. 将新数据从尾部插入到`keys`中
2. 每当缓存命中时（即缓存数据被访问），则将缓存数据移动到`keys`的尾部
3. 当`keys`长度大于max时，将头部的数据丢弃
